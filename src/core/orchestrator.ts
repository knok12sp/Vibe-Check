...
export async function scan(config: VibeCheckConfig): Promise<{ findings: Finding[]; summary: ScanSummary }> {
  ...
  let fingerprint = { framework: null as string | null, authProviders: [] as string[], aiGenerated: false, aiConfidence: 0 };
  if (hasRepo) {
  ...
  resetScanners();
  const allScanners = [
    secretsBasicScanner, debugFilesScanner, sourceMapsScanner,
    reactXSSScanner, authScanner, supabaseScanner, redirectsScanner,
    uploadsScanner, envExposureScanner,
    headersScanner, cspScanner, cookiesScanner, tlsScanner,
    routesScanner, crawlScanner,
    retireScanner, zapScanner, nucleiScanner, gitleaksScanner,
  ];
  ...
  const summary = generateSummary(findings, config.profile, config.repoPath || config.targetUrl || "unknown", duration, fingerprint.framework);

  logger.success(`Scan complete: ${summary.totalFindings} findings in ${duration.toFixed(1)}s (score: ${summary.score})`);
