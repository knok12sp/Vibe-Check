...
export async function reportCommand(file: string, opts: any, logger: Logger): Promise<void> {
  ...
  if (opts.json) {
    const outPath = typeof opts.json === "string" ? opts.json : "vibe-check-report.json";
  ...
  if (opts.md) {
    const outPath = typeof opts.md === "string" ? opts.md : "vibe-check-report.md";
  ...
  if (opts.html) {
    const outPath = typeof opts.html === "string" ? opts.html : "vibe-check-report.html";
    const { htmlReporter } = await import("../../reporters/html.js");
    await htmlReporter(summary, findings, outPath, logger);
  }
  if (opts.sarif) {
    const outPath = typeof opts.sarif === "string" ? opts.sarif : "vibe-check-report.sarif.json";
